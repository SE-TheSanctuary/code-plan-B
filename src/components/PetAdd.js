...
class PetAdd extends Component{
        ...
        return (
            <div className={className}>
                ...
                <input
                    ...
                    id={"addPet "+field.name} aria-describedby={field.name+"help"}
    ...
    onSubmit(values){
        ...
        console.log(values)
        if(this.props.user){
            firebase.database().ref('pets/' + this.props.user.uid).push().set({
                petName: values.shopname,
                petTel: values.phonenumber,
                petLocation: values.location,
                petAnimalType: values.animal,
                petCreatedBy: this.props.user.username
            })
            .then(function(){
                router.push('/petlist')
            })
